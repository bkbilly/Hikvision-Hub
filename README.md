...
- 🛡️ **Comprehensive In-Browser ISAPI Device Management**: Direct hardware and firmware configuration without proprietary plugins or Internet Explorer:
  - **Optics & Image Tuning**: Real-time adjustment of brightness, contrast, saturation, sharpness, Day/Night IR filter switching, Wide Dynamic Range (WDR), White Balance, Backlight / Highlight Compensation (BLC/HLC), 3D Digital Noise Reduction (DNR), 50/60Hz frequency, shutter speeds, gain levels, and 180° / mirror image flipping.
  - **Smart Events & VCA Detection**: Interactive SVG polygon/rectangle zone mapping on live snapshots for **Motion Detection** (auto-detecting Grid Map vs. Polygon Area, plus 90° Expert Rectangles across multiple areas), **Line Crossing** (with bidirectional arrows), **Intrusion Detection**, **Video Tampering**, **Region Entrance**, **Region Exiting**, **Unattended Baggage**, and **Object Removal** — complete with **Human and Vehicle Target Detection** filtering.
  - **Video Streams**: Inspect and tune Main (101) and Sub (102) stream resolutions, frame rates, bitrates, video codecs (H.264 / H.265), and I-frame intervals.
  - **Time & NTP Synchronization**: Synchronize camera clock with browser time in one click or configure remote NTP servers.
  - **Storage & SD Card Management**: Live HDD/SD health inspection, capacity/free space monitoring, and remote formatting via ISAPI.
  - **PTZ Controls**: Pan, tilt, zoom directional controls and preset position recall.
  - **Raw ISAPI Console**: Built-in diagnostic tool to test raw ISAPI endpoints directly from the browser.
...
- 📷 **Automatic Camera Snapshot & Picture Support**: Parses binary, SQLite, and HIKBTREE picture indices with zero-transcode direct JPEG slicing from `.pic` files and an inline photo viewer.
- 🔎 **Smart SQLite Indexing & Caching**: Crawls and indexes `info.bin`, `index00.bin` (binary), `HIKBTREE` (DVR binary B-tree), `record_db_index00` (SQLite), and picture databases for sub-millisecond timeline queries.
- 📡 **Multi-Camera Live View**: Auto-refreshing live snapshot grid with single-tap focus, ISAPI digest auth support, automatic pause when editing settings to save bandwidth, and full-screen monitoring.
...
4. To configure hardware optics, image adjustments, video streams, or smart event detection zones, click the **Device Settings** (sliders icon) next to any camera in the list or live view.
...
- **Video Storage & Indices**: `hiv00000.mp4` via `index00.bin` (binary format), `HIKBTREE` (DVR binary B-tree format), and `record_db_index00` (SQLite format)
- **Picture Storage & Indices**: `hiv00000.pic` via `index00p.bin` (binary format), `HIKBTREE` (DVR binary B-tree format), and `event_db_index00` (SQLite format)
...
- **Direct ISAPI Camera Management**: Integrated full hardware control and smart events mapping (Motion, Line Crossing, Intrusion, VCA targets, etc.) directly into the web interface.
